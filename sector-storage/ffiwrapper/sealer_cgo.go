...
//TODO: TO BE CHANGED? BUT I THINK IT NEED NOT TO BE CHANGED.
...
// Now the preCommit phase doing the following things: copy the temp file of the sector into the seal and unseal path; use the (sector data)+(ticket) as the pc1 output
func (sb *Sealer) SealPreCommit1(ctx context.Context, sector storage.SectorRef, ticket abi.SealRandomness, pieces []abi.PieceInfo) (out storage.PreCommit1Out, err error) {
	...
	ssize, err := sector.ProofType.SectorSize() //this is the maximum size of the sector
	if err != nil {
		return nil, err
	}
	...
	if bExist, _ := storiface.FileExists(tUnsealedFile); bExist {
		if bExist, _ := storiface.FileExists(paths.Unsealed); !bExist {
			...
			err = storiface.CopyFile(tUnsealedFile, paths.Unsealed) //copy the temp sector file into the unseal sector file path
			if err != nil {
				return nil, err
			}
		}
	...
	err = storiface.CopyFile(tUnsealedFile, paths.Sealed) //copy the temp sector file into the seal sector file path
	if err != nil {
		return nil, err
	}
...
//use XOR to avoid the position of the Merkle tree

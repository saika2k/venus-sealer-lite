...
var fsmPlanners = map[types.SectorState]func(events []statemachine.Event, state *types.SectorInfo) (uint64, error){
	...
	types.PreCommit1: planOne( //copy the unsealed sector data in to sealed sector path, generate the SDR,however we dont need the SDR anymore
	...
	types.PreCommit2: planOne( //generate the commD and commR, as in storagelite, the commD is same as the commR
		on(SectorPreCommit2{}, types.PreCommitting),
		on(SectorSealPreCommit2Failed{}, types.SealPreCommit2Failed),
		on(SectorSealPreCommit1Failed{}, types.SealPreCommit1Failed),
		on(SectorOldTicket{}, types.GetTicket),
	),
	types.PreCommitting: planOne( //commit some parameters to the messager and then wait for the message be put on chain.
	...
	types.WaitSeed: planOne( //now compute the real PoRep and then commit the PoRep, finally the sector is in the proving stage, where PoSts are created

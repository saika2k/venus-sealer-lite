...
func (m *Sealing) preCommitParams(ctx statemachine.Context, sector types.SectorInfo) (*miner.SectorPreCommitInfo, big.Int, types.TipSetToken, error) {
	...
	params := &miner.SectorPreCommitInfo{
		...
		ReplaceCapacity: saveinc, // ReplaceCapacity is used for record whether the sector save a file increment, put this message online for the PoSt process
	}
...
func (m *Sealing) handlePreCommitting(ctx statemachine.Context, sector types.SectorInfo) error {
	...
	// the parms are encoded in the varience 'enc'. the enc will be committed to the messager, the precommit on chain infomation include
	// the type SectorPreCommitInfo, which is the type of the parms, thus, using the API StateSectorPreCommitInfo, we could get the
	// information of the tateSectorPreCommitInfo, thus get the SectorPreCommitInfo, then we can check whether the sector contains a
	// file increment or not by check the ReplaceCapacity.
